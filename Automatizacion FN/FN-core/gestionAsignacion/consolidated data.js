function actualizeQuantityByMonth() {
  ...
  for (const pyUser of uniqueUsers) {
    ...
    //   // console.log({ pyUser, leng: filterByUser.length })
  ...
  //   averageRaw: userData.quantity > 0 ? Math.floor(userData.total / userData.quantity) : 0,
  //   average: userData.quantity > 0 ? secondsToHourFormat(Math.floor(userData.total / userData.quantity)) : "0:00:00",
  //   total: userData.total > 0 ? secondsToHourFormat(userData.total) : "0:00:00",
  // }))

  console.log({ quantityByUser })

  const placeFormatData = quantityByUser.map(data => [data.user, data.month, /* data.average, data.total, */ data.quantity /*, data.workedDays */])
