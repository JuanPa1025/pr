function actualizeQuantityByMonth() {
  ...
  for (const pyUser of uniqueUsers) {
    const filterByUser = [];
    const rawUserData = consolidateData.filter(data => data[5] === pyUser && (parseInt(data[0].split('/')[1])) === lastMonth + 1);
    for (const row of rawUserData) {
      const id = row[2];
      const idAlreadyExist = filterByUser.findIndex(item => item[2] === id);
      if (idAlreadyExist === -1) {
        filterByUser.push(row);
      }
    }
    console.log({
      filterByUser: filterByUser.length,
      rawUserData: rawUserData.length,
      user: pyUser,
    })
